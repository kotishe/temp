...
World::World()
{
    ...
    QueryResult *result = LoginDatabase.Query("SELECT `maxplayers` FROM uptime ORDER BY `maxplayers` DESC LIMIT 1 ");
    if( result ){
...
void World::LoadConfigSettings(bool reload)
{
    ...
    setConfig(CONFIG_UINT32_WARDEN_BAN_TIME, "Warden.BanLength", 1);
}
